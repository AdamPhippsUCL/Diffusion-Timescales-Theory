...
for dindx = 1:length(deltas)
    ...
    if dindx == 1
        ...
        plot(Deltas, alpha*(thisDeltas-delta/3), '--', color = 'k', DisplayName='$\Delta - \frac{\delta}{3}$', HandleVisibility='off')
...
plot([tau, tau], [0, phase_vars1(indx)], '--', color = 'k', HandleVisibility='off')

[~, indx] = min(abs(deltas-2*tau));
plot([2*tau, 2*tau], [0, phase_vars1(indx)], '--', color = 'k', HandleVisibility='off')
...
%% Look at change with delta

f1=figure;
f1.Position  = [-1038 193 862 676];
ax1 = axes;
...
for Delta =Deltas(inds)
    ...
    y = phase_vars(:,Deltaindx);
    beta = phase_vars(1,1)/(Deltamin-deltamin/3);
    beta=alpha;

    % Plot
    plot(ax1, deltas(y~=0), y(y~=0), Color = cmap(ceil(256*Delta/(2*tau)), :), LineWidth = 1);
    hold(ax1, 'on');
    yline(phase_var_sat, '--')


    thisy = y(y~=0);
    last_points = [last_points, thisy(end)];
...
xlabel('$\delta \rightarrow$', Position=[46,-0.002],Interpreter='latex', FontSize=20)
ylabel('$\langle \Delta\phi^2 \rangle \rightarrow$', Position=[-0.64 0.144] , Interpreter='latex', FontSize=20)

saveas(f1, 'sphere_phase_var_small_delta.png')